...
class ReolinkAPI:
    ...
    def login(self) -> None:
        ...
        post_data = {"User": {"Version": 0, "userName": self.username, "password": self.password}}

        try:
            response = self.session.post(
                f"{self.base_url}/api.cgi?cmd=Login", json=[{"cmd": "Login", "action": 0, "param": post_data}]
            )
            ...
            raise Exception(f"Failed to connect to camera: {e!s}")
    ...
    def set_ptz_ctrl(self, command: str, channel: int = 0, speed: int = 1) -> dict:
        ...
        Returns:
            dict: Response from the camera indicating success or failure
        """
        url_params = {"cmd": "PtzCtrl", "token": self.token}
...
if __name__ == "__main__":
    ...
    ai_config_params = {
        "aiTrack": 1,
        "trackType": {"dog_cat": 0, "face": 1, "people": 1, "vehicle": 0},
        "AiDetectType": {"people": 1, "vehicle": 0, "dog_cat": 0, "face": 1},
    }
