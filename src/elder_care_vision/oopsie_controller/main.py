...
def create_frame_sequence(frames, timestamps, width=None):
    ...
    for i, (frame, timestamp) in enumerate(zip(frames, timestamps, strict=False)):
        row = i // 3
        col = i % 3
...
def on_confirmed_fall(sequence_frames, sequence_timestamps, analysis, timestamp):
    ...
    try:
        ...
        cv2.putText(
            padded_sequence, f"Analysis: {analysis}", (padding, y_pos), font, font_scale, (255, 255, 255), thickness
        )
        ...
        logging.exception(f"Error processing confirmed fall: {e!s}")
...
def process_video(controller: OopsieController, video_path: str) -> None:
    ...
    try:
        ...
        print(f"Error processing video: {e!s}")
